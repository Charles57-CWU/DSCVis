...
class MakePlot(QOpenGLWidget):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.RightButton:
            x = self.m_left + (event.pos().x() * (self.m_right - self.m_left)) / self.width
            y = self.m_bottom + ((self.height - event.pos().y()) * (self.m_top - self.m_bottom)) / self.height
            # print(str(x) + " " + str(y))
            self.rect.append(x)
            self.rect.append(y)
            if len(self.rect) == 4:
    ...
    def wheelEvent(self, event):
        ...
        mouseX = event.pos().x() / self.width
        mouseY = (1 - event.pos().y()) / self.height
